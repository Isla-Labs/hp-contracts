...
import { BaseDopplerHookMigrator } from "@doppler/src/base/BaseDopplerHookMigrator.sol";
import { MigrationMath } from "@doppler/src/libraries/MigrationMath.sol";
import { DopplerHookMigrator } from "@doppler/src/migrators/DopplerHookMigrator.sol";
import { Position } from "@doppler/src/types/Position.sol";
import {
...
/// @dev hookData fee mode: `0` / empty = full fee; `1` = half fee (whitelisted routers only).
uint8 constant FEE_MODE_FULL = 0;
uint8 constant FEE_MODE_HALF = 1;

/// @notice Emitted when a router is allowed/disallowed to request half-fee swaps via hookData.
event FeeDiscountRouterSet(address indexed router, bool allowed);
 ...
 * @dev HighPotential fork: whitelisted routers may pass hookData `0x01` for half fee (player↔player hops).
 */
contract RehypeDopplerHookMigrator is BaseDopplerHookMigrator, ReentrancyGuard {
    ...
    /// @notice Routers allowed to request `FEE_MODE_HALF` via hookData (e.g. TradeRouter).
    mapping(address router => bool allowed) public isFeeDiscountRouter;
    ...
    /**
     * @notice Allow/disallow a swap router to request half fee via hookData `0x01`.
     * @dev Airlock owner only. Empty / `0x00` / unknown modes always charge full fee (fail closed).
     */
    function setFeeDiscountRouter(address router, bool allowed) external {
        address airlockOwner = MIGRATOR.airlock().owner();
        require(msg.sender == airlockOwner, SenderNotAirlockOwner());
        require(router != address(0), SenderNotAuthorized());
        isFeeDiscountRouter[router] = allowed;
        emit FeeDiscountRouterSet(router, allowed);
    }
    ...
    function _onAfterSwap(
        ...
        (Currency feeCurrency, int128 hookDelta) = _collectSwapFees(params, delta, key, poolId, sender, hookData);
     ...
     * @param sender Swap initiator (PoolManager `sender`; typically the router)
     * @param hookData Fee mode: empty/`0x00` = full; `0x01` = half if `sender` is whitelisted
    ...
    function _collectSwapFees(
        ...
        uint24 effectiveFee = _applyFeeMode(getHookFees[poolId].customFee, sender, hookData);
        uint256 feeAmount = FullMath.mulDiv(feeBase, effectiveFee, MAX_SWAP_FEE);
    ...
    /**
     * @dev Fail-closed fee mode: half fee only when `sender` is whitelisted and hookData is exactly `0x01`.
     */
    function _applyFeeMode(uint24 baseFee, address sender, bytes calldata hookData) internal view returns (uint24) {
        if (hookData.length == 1 && uint8(hookData[0]) == FEE_MODE_HALF && isFeeDiscountRouter[sender]) {
            return baseFee / 2;
        }
        return baseFee;
    }
