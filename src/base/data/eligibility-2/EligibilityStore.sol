...
/**
 * @title EligibilityStore (eligibility-2)
 * @notice Standalone data plane: CVM squads oracle + PpmVerifier appearance ingest.
 * @dev Deployed behind its own proxy (EIP-170). Write paths (thresholds / cohort routing
 *      live on `EligibilityVerifier`):
 *
 *      1) `requestFetch` → `CvmJob.SquadSync` / `HistoricalSquadSync` → `_fulfillRequest`
 *         → `_processReport` (FETCH / SYNC phases)
 *      2) `continueSort` → `_sortTransient` (gas-chunked; no HTTP)
 *      3) `syncLeagueFromRegistry` → `_syncLeague` (TournamentRegistry; no HTTP)
 *      4) PpmVerifier → `recordAppearances` (career `positionMinutes` + per-league score)
 *
 ...
 */
contract EligibilityStore is Initializable, AddressBook, Ownable, Oracle {
    // --------------------------------------------
    //  Constants
    // --------------------------------------------
    ...
    /// @notice When the last current-season pass finished (resweep throttle input).
    uint64 public lastCurrentPassCompletedAt;

    /// @notice In-flight CVM fetch request (`bytes32(0)` when idle).
     ...
     * @dev `CVM_ROUTER` must already be registered (oracle set first).
    ...
    /**
     * @notice Proxy init: ownership, year clocks, verifier wire.
     ...
     */
    function initialize(uint16 baseYear_, address eligibilityVerifier_) external initializer {
    ...
    /// @notice Manually queue a league's seasons (oldest→newest). Prefer `syncLeagueFromRegistry`.
    ...
    // --------------------------------------------
    //  Oracle kickoffs — permissionless
    // --------------------------------------------

    /**
     * @notice Open a CVM fetch for the next fetchable page (active season or cron current).
     * @dev Args: `abi.encode(FETCH_TRANSIENT, leagueId, seasonId, pageToFetch, personsOffset)`.
     *      Job: `HistoricalSquadSync` while historical pass is active, else `SquadSync`.
     *      Response must ABI-decode as `SquadReport` (FETCH / SYNC). One request at a time.
     */
    function requestFetch() external returns (bytes32 requestId) {
        if (pendingRequestId != bytes32(0)) revert Errors.OracleRequestPending(pendingRequestId);

        (CvmJob job, SquadRequest memory req) = _nextFetchRequest();
        requestId = _sendOracleRequest(
            job, abi.encode(req.phase, req.leagueId, req.seasonId, req.pageToFetch, req.personsOffset)
        );
        pendingRequestId = requestId;

        emit Events.SquadFetchRequested(
            requestId, req.leagueId, req.seasonId, req.pageToFetch, req.personsOffset
        );
    }

    /**
     * @notice Gas-chunked SORT for the active `TRANSIENT` season (no oracle / HTTP).
     * @dev Replaces the CRE empty `SORT_TRANSIENT` report. Repeat until finalize.
     */
    function continueSort() external {
        WorkflowControl storage c = _control;
        if (c.pass == PassKind.None) revert Errors.NoActivePass();

        SquadReport memory r;
        r.phase = SquadPhase.SORT_TRANSIENT;
        r.leagueId = c.activeLeagueId;
        r.seasonId = c.activeSeasonId;
        _sortTransient(r);
    }

    /**
     * @notice Reconcile `RunBook` with onchain TournamentRegistry seasons for `leagueId`.
     * @dev Same effect as a CRE/CVM `SYNC_LEAGUE` report, without an oracle round-trip.
     */
    function syncLeagueFromRegistry(bytes32 leagueId) external {
        if (leagueId == bytes32(0)) revert Errors.ZeroId();
        (bytes32[] memory seasonIds, uint16[] memory seasonStartYears) =
            _tournamentRegistry().getSeasonsOldestFirst(leagueId);
        if (seasonIds.length == 0) revert Errors.NoQueuedWork();
        _syncLeague(leagueId, seasonIds, seasonStartYears);
    }
    ...
    /**
     * @notice Current-pass gate: next `IDLE` season for `currentSeasonStartYear`, if a pass may start.
    ...
    // --------------------------------------------
    //  CVM fulfill — phase dispatch
    // --------------------------------------------

    /// @inheritdoc Oracle
    function _fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
        if (pendingRequestId != requestId) revert Errors.UnknownOracleRequest(requestId);
        delete pendingRequestId;

        if (err.length != 0 || response.length == 0) {
            emit Events.SquadOracleFailed(requestId, err);
            return;
        }

        _processReport(abi.decode(response, (SquadReport)));
    }

    /**
     * @dev Former CRE `_processReport` body: dispatch by `SquadReport.phase`.
     *      FETCH / SYNC arrive from CVM fulfills; SORT is also accepted if a job returns it,
     *      but keepers should prefer the cheaper `continueSort()` path.
     */
    function _processReport(SquadReport memory r) private {
    ...
    /**
     * @dev SeasonOpened / registry reconcile into the RunBook.
    ...
    /**
     * @dev Appends one SP page-slice. Stay-on-page drains a club across fulfills via
     *      `personsOffset`; `_pgNm` only advances when the oracle marks the club complete
    ...
    /**
     * @dev Append one newer season under an already-queued league (registry divergence).
    ...
    // --------------------------------------------
    //  Internal — fetch request encoding
    // --------------------------------------------

    /**
     * @dev Build the next FETCH job + cursor from workflow / transient state.
     *      Active IDLE season → resume page/offset; else start next current-year IDLE season.
     */
    function _nextFetchRequest() private view returns (CvmJob job, SquadRequest memory req) {
        WorkflowControl storage c = _control;
        req.phase = SquadPhase.FETCH_TRANSIENT;

        if (c.pass != PassKind.None && c.activeSeasonId != bytes32(0)) {
            SeasonRun storage row = _runBook.seasons[c.activeLeagueIndex][c.activeSeasonIndex];
            if (row.status != RunStatus.IDLE) revert Errors.SeasonNotFetchable(c.activeSeasonId);

            req.leagueId = c.activeLeagueId;
            req.seasonId = c.activeSeasonId;

            TransientReturn storage buf = _transient;
            if (buf.seasonId == req.seasonId) {
                uint16 page = buf.nextPage == buf.pageFetched ? buf.pageFetched : buf.nextPage;
                if (page == 0 || page >= SQUAD_FETCH_PAGE_DONE) {
                    revert Errors.SeasonNotFetchable(req.seasonId);
                }
                req.pageToFetch = page;
                req.personsOffset = buf.personsOffset;
            } else {
                req.pageToFetch = 1;
                req.personsOffset = 0;
            }

            job = c.pass == PassKind.Historical ? CvmJob.HistoricalSquadSync : CvmJob.SquadSync;
            return (job, req);
        }

        if (c.pass != PassKind.None || c.currentSeasonStartYear == 0) revert Errors.NothingToFetch();

        (bool found, uint16 li, uint16 si) = _findIdleSeason(0, 0);
        if (!found) revert Errors.NothingToFetch();

        req.leagueId = _runBook.leagueIds[li];
        req.seasonId = _runBook.seasons[li][si].seasonId;
        req.pageToFetch = 1;
        req.personsOffset = 0;
        job = CvmJob.SquadSync;
    }
