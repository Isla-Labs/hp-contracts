...
// =============================================================================
//  EligibilityTypes (eligibility-2)
//
//  Same season state machine as eligibility v1, but offchain datasets arrive via
//  the custom CVM oracle (`Oracle._sendOracleRequest` → `_fulfillRequest`) instead
//  of CRE Keystone `onReport`.
//
//  Kickoff surface:
//    - `requestFetch`     → CvmJob.SquadSync / HistoricalSquadSync (HTTP page slice)
//    - `continueSort`     → pure onchain gas chunks (no HTTP)
//    - `syncLeagueFromRegistry` → onchain TournamentRegistry reconcile (no HTTP)
//
//  Unit of work = one seasonId at a time:
//    1) Append FETCH page-slices into TransientReturn until the season is complete
//       (SP page ≈ one club; oracle may stay-on-page and drain that club in slices)
//    2) SORT the full season snapshot: upsert MinutesStore → rebuild SquadList →
//       removal pass (absent from TransientReturn membership)
//    3) Clear buffer; auto-advance to the next seasonId (onchain — no offchain bookkeeping)
// =============================================================================
...
/// @notice Oracle fulfill / phase discriminator (same ABI as v1 CRE `SquadReport.phase`).
/// @dev `FETCH_TRANSIENT` carries a **page slice** (store appends).
///      `SORT_TRANSIENT` is onchain-only (gas-chunked via `continueSort` / `SortPage`).
...
/// @dev `historicalActive == (pass == Historical)`. Cron / `requestFetch` no-ops unless
///      `pass == None` (current) or an active IDLE season is fetchable. Exactly one
///      `(activeLeagueId, activeSeasonId)` in flight.
 ...
 * @dev Built incrementally: SP `_pgNm` ≈ one club; oracle may split that club across
 *      multiple fulfills (**stay-on-page**). `personsOffset` is the within-page
 *      cursor that makes stay-on-page idempotent under retries.
 *
 ...
 *
 *      Sort rules (`SORT_TRANSIENT` / `continueSort`):
...
struct TransientReturn {
    ...
    uint16 pageFetched;
    /// @dev Next SP page the oracle should request, or `SQUAD_FETCH_PAGE_DONE`.
    uint16 nextPage;
...
/// @notice CVM request args for `SquadSync` / `HistoricalSquadSync` (FETCH path).
/// @dev Worker ABI: `abi.encode(phase, leagueId, seasonId, pageToFetch, personsOffset)`.
///      SORT / SYNC are not sent as CVM jobs from the store (onchain kickoffs).
struct SquadRequest {
    SquadPhase phase;
    bytes32 leagueId;
    bytes32 seasonId;
...
/// @notice Unified oracle fulfill decode target (same layout as v1 CRE report).
...
// --------------------------------------------
//  Squad workflow — events (keeper / indexer triggers)
// --------------------------------------------

/// @dev Emitted by EligibilityStore. Topic0 drives offchain keepers that call
///      `requestFetch` / `continueSort`. League/season advance is onchain-automated.
library SquadWorkflowEvents {
    ...
    ///      (or `LoopPending` if the book is terminal).
    event PopulationComplete(bytes32 indexed leagueId);

    /// @notice Entire `RunBook` is terminal (`IDLE`|`ARTIFACT`) and `pass == None`.
    /// @dev Optional wake; hourly / daily `requestFetch` remains the steady-state heartbeat.
