...
interface IPlayerVault {
    ...
    /// @notice Snapshot staked balances for `(tournamentId, seasonId, roundNumber)` at mwStartTime
    function snapshot(bytes32 tournamentId, uint16 seasonId, uint32 roundNumber) external returns (uint256 snapId);
}
