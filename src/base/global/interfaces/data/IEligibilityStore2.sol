...
/**
 * @title IEligibilityStore2
 * @notice CVM-oracle squads data plane + minutes / score storage (eligibility-2).
 * @dev Intended behind `TransparentUpgradeableProxy`. Keepers → `requestFetch` /
 *      `continueSort` / `syncLeagueFromRegistry`; CVM → `_fulfillRequest`; PpmVerifier →
 *      `recordAppearances`; EligibilityVerifier → privileged verify helpers.
 */
interface IEligibilityStore2 {
    function initialize(uint16 baseYear_, address eligibilityVerifier_) external;
    ...
    /// @notice Open next CVM FETCH (`SquadSync` / `HistoricalSquadSync`).
    function requestFetch() external returns (bytes32 requestId);

    /// @notice Gas-chunked SORT for the active TRANSIENT season (no oracle).
    function continueSort() external;

    /// @notice Reconcile RunBook from TournamentRegistry seasons (no oracle).
