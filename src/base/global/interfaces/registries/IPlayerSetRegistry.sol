...
interface IPlayerSetRegistry {
    ...
    // --------------------------------------------
    //  Status lifecycle — MigrationListener / LifecycleManager
    // --------------------------------------------

    /**
     * @notice Bonding → graduated: write migrated `activePool` + `GRADUATED`.
     * @dev Caller: `MIGRATION_LISTENER`. Mirrors status onto FeeRouter (fee split).
     *      No vault / tournament membership fan-out.
     */
