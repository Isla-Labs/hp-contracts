...
contract EligibilityVerifier is Initializable, EligibilityCriteria, CreReceiver, RateLimit {
     ...
     *         Deployed + below continuity → `toDeactivate`
    ...
    function verifyEligibility(
        ...
        groups.toDeactivate = new bytes32[](deactCount);
        ...
        uint256 deactIdx;
        uint256 reactIdx;
        uint32[] memory deactMins = new uint32[](deactCount);
        ...
        for (uint256 i = offset; i < end; ++i) {
            ...
            if (playerSetRegistry.playerExists(playerId)) {
                ...
                groups.toDeactivate[deactIdx] = playerId;
                deactMins[deactIdx] = effectiveMins;
                unchecked {
                    ++deactIdx;
                }
        ...
        if (deactCount != 0) {
            transferLocker.enqueueLifecycle(groups.toDeactivate, LifecycleReason.ContinuityUnderThreshold, deactMins);
        }
        if (reactCount != 0) {
