...
/**
 * @title PbrTreasury
 * @notice Single-cup PBR pot: fee accrual, lock / snapshot / settle, pull claims.
 * @dev One deployment per `cupId`. Fees arrive from `PbrFeeHub` (or directly).
 *
...
contract PbrTreasury is Initializable, AccessControl, ReentrancyGuard {
     ...
     * @param competitionId_ League / Continental / International id.
     * @param cupId_ Sole cup this treasury serves.
     * @param initialSeason_ Starting season (e.g. 2025).
     */
    function initialize(address admin_, bytes32 competitionId_, bytes32 cupId_, uint16 initialSeason_)
        external
        initializer
    {
        if (admin_ == address(0)) revert ZeroAddress();
        if (competitionId_ == bytes32(0) || cupId_ == bytes32(0)) revert ZeroId();
    ...
    /// @notice Freeze `rewardsR` into the active round when published and due.
    function lock() external {
        ...
        if (!tournamentRegistry.isRoundPublished(competitionId, cupId, season, roundNumber)) revert NothingDue();

        RoundSchedule memory schedule = tournamentRegistry.getRound(competitionId, cupId, season, roundNumber);
        if (block.timestamp < schedule.startTime) revert NothingDue();

        uint32 finalRound = tournamentRegistry.getFinalRound(competitionId, cupId, season);
    ...
    function snapshotBatch() external returns (uint256 processed, bool done) {
        ...
        for (uint256 i = cursor; i < end;) {
            ...
            if (playerVault.snapIdOf(cupId, season, roundNumber) == 0) {
                uint256 snapId = playerVault.snapshot(cupId, season, roundNumber);
