...
contract TradeRouter2Test is Test {
    ...
    function test_buy_exactIn_oneToOne() public {
        ...
        assertEq(hooks.lastHookData(), hex"00");
    }
    ...
    function test_swapPlayers_exactIn_oneToOne() public {
        ...
        assertEq(address(router).balance, 0);
        // initial buy + sell leg + buy leg
        assertEq(hooks.beforeSwapCount(), 3);
        assertEq(hooks.afterSwapCount(), 3);
        // Both Doppler legs of swapPlayers request Rehype half-fee mode.
        assertEq(hooks.lastHookData(), hex"01");
    }
